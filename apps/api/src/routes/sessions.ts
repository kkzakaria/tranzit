...
router.delete('/:id', async (c) => {
  ...
  try {
    ...
    console.error('[sessions] revokeSession a échoué', { token, err })
    return c.json({ error: 'Révocation de session échouée' }, 500)
  }

  // Audit après la révocation — si cela échoue, la session est quand même révoquée
  try {
    ...
    console.error('[sessions] écriture audit échouée après révocation réussie', err)
  }
...
router.delete('/', async (c) => {
  ...
  try {
    ...
    console.error('[sessions] revokeSessions a échoué', err)
    return c.json({ error: 'Révocation des sessions échouée' }, 500)
  }
